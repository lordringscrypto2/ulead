...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ULEAD);
    ...
    if (!settings.contains("nAnonymizeULeadAmount"))
        settings.setValue("nAnonymizeULeadAmount", 1000);

    nAnonymizeULeadAmount = settings.value("nAnonymizeULeadAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeULeadAmount"))
        SoftSetArg("-anonymizeuleadamount", settings.value("nAnonymizeULeadAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ulead.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeULeadAmount:
            return QVariant(nAnonymizeULeadAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeULeadAmount:
            nAnonymizeULeadAmount = value.toInt();
            settings.setValue("nAnonymizeULeadAmount", nAnonymizeULeadAmount);
            emit anonymizeULeadAmountChanged(nAnonymizeULeadAmount);
            break;
        case CoinControlFeatures:
