...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ULEAD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zULEAD send
enum ZerocoinSpendStatus {
    ZULEAD_SPEND_OKAY = 0,                            // No error
    ZULEAD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZULEAD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZULEAD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZULEAD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZULEAD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZULEAD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZULEAD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZULEAD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZULEAD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZULEAD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZULEAD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZULEAD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZULEAD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZULEAD_SPENT_USED_ZULEAD = 14,                      // Coin has already been spend
    ZULEAD_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZULEAD_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZULEADOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzuleadAuto) const;
    ...
    */
    ...
    CzULEADWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzULEADTracker> zuleadTracker;
    ...
    void setZWallet(CzULEADWallet* zwallet)
    {
        zwalletMain = zwallet;
        zuleadTracker = std::unique_ptr<CzULEADTracker>(new CzULEADTracker(strWalletFile));
    }

    CzULEADWallet* getZWallet() { return zwalletMain; }
