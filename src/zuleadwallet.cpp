...
CzULEADWallet::CzULEADWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zulead seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZULEADSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZULEADSeed_deprecated()) {
                    LogPrintf("%s: Updated zULEAD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zulead seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zulead wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzULEADWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZULEADCount(nCountLastUsed);
    else if (!walletdb.ReadZULEADCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzULEADWallet::Lock()
{
    seedMaster = 0;
}

void CzULEADWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzULEADWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZULEAD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzULEADWallet::LoadMintPoolFromDB()
{
...
void CzULEADWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzULEADWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzULEADWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zuleadTracker->HasPubcoinHash(pMint.first)) {
...
bool CzULEADWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZULEAD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zuleadTracker which also adds to database
    pwalletMain->zuleadTracker->Add(dMint, true);
...
void CzULEADWallet::SeedToZULEAD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzULEADWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzULEADWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZULEADCount(nCountLastUsed);
...
void CzULEADWallet::GenerateDeterministicZULEAD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzULEADWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzULEADWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
