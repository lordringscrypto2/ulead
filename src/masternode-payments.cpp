    ...
    if (strCommand == NetMsgType::MNGET) { //Masternode Payments Request Sync
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (pfrom->HasFulfilledRequest(NetMsgType::MNGET)) {
				...
				LogPrintf("$$$$--- Misbehaving(pfrom->GetId(), 20); is calling from masternode-payment.cpp 366 pfrom->HasFulfilledRequest(NetMsgType::MNGET)true \n");
        ...
        if (!winner.SignatureValid()) {
            ...
            if (masternodeSync.IsSynced()) {
				LogPrintf("$$$$--- Misbehaving(pfrom->GetId(), 20); is calling from masternode-payments 414- issynce true \n");
				Misbehaving(pfrom->GetId(), 20);}
