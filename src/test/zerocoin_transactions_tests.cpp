...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zuleadTracker = unique_ptr<CzULEADTracker>(new CzULEADTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZULEAD_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZULEAD_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
