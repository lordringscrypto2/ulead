...
CzULEADTracker::CzULEADTracker(std::string strWalletFile)
{
...
CzULEADTracker::~CzULEADTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzULEADTracker::Init()
{
...
bool CzULEADTracker::Archive(CMintMeta& meta)
{
...
bool CzULEADTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzULEADTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzULEADTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzULEADTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzULEADTracker::GetSerialHashes()
{
...
CAmount CzULEADTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzULEADTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzULEADTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzULEADTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzULEADTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzULEADTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzULEADTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzULEADTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzULEADTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzULEADTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzULEADTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzULEADTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzULEADTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzULEADTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzULEADTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzULEADTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzULEADTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzulead from DB\n", __func__, listDeterministicDB.size());
    }
