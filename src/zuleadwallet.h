...
#ifndef ULEAD_ZULEADWALLET_H
#define ULEAD_ZULEADWALLET_H
...
public:
    CzULEADWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZULEAD(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZULEAD(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ULEAD_ZULEADWALLET_H
